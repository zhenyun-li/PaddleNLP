...
void DecoderWriteCacheKV(
  ...
  const uint32_t elem_nums =
      bsz * kv_num_heads * (head_dim_qk + head_dim_v);
  ...
  append_decode_cache_T_kernel<T, PackSize><<<grid_size, blocksize, 0, stream>>>(
