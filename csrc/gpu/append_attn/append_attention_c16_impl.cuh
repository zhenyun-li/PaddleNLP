...
__global__ void multi_query_append_attention_kernel(
  ...
  const uint32_t q_ori_n_stride =  q_num_heads * HEAD_DIM_QK + kv_num_heads * HEAD_DIM_QK + kv_num_heads * HEAD_DIM_V;
...
#ifdef DEBUG_PERCISION
    __syncthreads();
    if (threadIdx.x == 0 && threadIdx.y == 0 && blockIdx.z == 0 && blockIdx.x == 0) {
...
__global__ void multi_query_append_attention_warp1_4_kernel(
  ...
  smem_t k_smem(smem + num_frags_x * 16 * HEAD_DIM_QK * sizeof(T)),
      v_smem(smem + (num_frags_x + NUM_WARP_KV * num_frags_z) * 16 * HEAD_DIM_QK *
                        sizeof(T));
...
void MultiQueryAppendAttention(
  ...
  if constexpr (NUM_WARP_Q == 4) {
    ...
    constexpr uint32_t smem_size =
        ...
        NUM_WARP_KV * num_frags_z * HEAD_DIM_V) * 16 * sizeof(T);
